...
.LFB0:
	...
	movsd	xmm2, xmm0			# x - первый аргумент функции
	...
	movsd	xmm0, xmm2			# pow_x = x;
	movsd	xmm5, xmm0
	
	jmp	.L2				# while (fabs(now - prev) > 0.0005) {
.L3:

	movsd	xmm0, xmm4			# 	prev = now;
	movsd	xmm3, xmm0

	movsd	xmm0, xmm4			# 	now += pow_x;
	addsd	xmm0, xmm5
	movsd	xmm4, xmm0

	movsd	xmm0, xmm5			# 	pow_x *= x;
	mulsd	xmm0, xmm2
	movsd	xmm5, xmm0
.L2:
